...
-- Create a function to check if an object is in temp-uploads
CREATE OR REPLACE FUNCTION storage.is_temp_upload()
RETURNS trigger AS $$
BEGIN
  IF NEW.bucket_id = 'temp-uploads' THEN
...
CREATE TRIGGER cleanup_temp_uploads_trigger
  ...
  FOR EACH ROW
  EXECUTE FUNCTION storage.is_temp_upload();